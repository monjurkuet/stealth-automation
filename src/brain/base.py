...
class BaseAutomation(ABC):
    ...
    def __init__(self, bridge: HTTPBridge, platform: str):
        self.bridge = bridge
    ...
    async def _wait_for_results(self):
        ...
        result = await self.bridge.get_result(
            wait_id,
    ...
    async def _navigate(self, url: str):
        ...
        logger.info(f"Navigate result for {url}: {result}")

        if result.get("status") != "success":
            raise Exception(f"Navigation failed: {url} - Result: {result}")
